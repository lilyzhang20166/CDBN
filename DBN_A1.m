...
opts.momentum  = 0;
opts.alpha     = 1;
dbn = dbnsetup(dbn, A1SRRR, opts);
dbn = dbntrain(dbn, A1SRRR, opts);
%unfold dbn to nn
...
[er, bad] = nntest(nn, A1SRRR, A1SRRR);
...
Xz=A1SRRR*x1*x2;
save A1Xz Xz
x=A1SRRR*x1*x2*x3*x4;
xx=reshape(x,a1,a2,[]);

xxs=reshape(A1SRRR,a1*a2,[]);
for i=1:a1*a2
    xx0(i)=norm(xxs(i,:)-x(i,:))^2;
end

xxzr=reshape(xx0,a1,a2);
save A1Xzresult xxzr;
kbs0=xxzr;
figure,imshow(xxzr,[])   %  Visualize the RBM weights
